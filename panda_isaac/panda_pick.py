...
class PandaPickEnv(BaseTask):
	def __init__(self, cfg, physics_engine=gymapi.SIM_PHYSX, sim_device="cuda:0", headless=False):
		...
		middle_env = self.envs[self.num_envs //
														2 + int(np.sqrt(self.num_envs)) // 2]
		...
		self.im_mean = torch.tensor(
			[0.485, 0.456, 0.406], dtype=torch.float, device=self.device).view(3, 1, 1)
		self.im_std = torch.tensor(
			[0.229, 0.224, 0.225], dtype=torch.float, device=self.device).view(3, 1, 1)
	...
	def _create_envs(self):
		...
		# print("box asset options", asset_options.density)  # 1000
		box_asset = self.gym.create_box(
			self.sim, box_size, box_size*4, box_size, asset_options)
		asset_options.fix_base_link = True 
		goal_asset = self.gym.create_box(
			self.sim, box_size, box_size, box_size, asset_options)
		...
		if controller == "ik":
			...
			franka_dof_props["stiffness"][:7] = np.array(
				[80.0, 120.0, 100.0, 100.0, 70.0, 50.0, 20.0])
			franka_dof_props["damping"][:7] = np.array(
		...
		self.franka_num_dofs = franka_num_dofs = self.gym.get_asset_dof_count(
			franka_asset)
		default_dof_pos = np.zeros((2, franka_num_dofs), dtype=np.float32)
		# "ready" pos
		# default_dof_pos[:7] = np.array([0.0, -0.785, 0.0, -2.356, 0.0, 1.571, 0.785])
		# [0.4, 0.0, 0.7] pos
		default_dof_pos[...,:7] = np.array(
			[0.0, -0.22146, 0.0, -2.7018, 0.0, 2.4804, 0.78533])
		# grippers open
		default_dof_pos[...,7:] = franka_upper_limits[7:]

		default_dof_state = np.zeros((2, franka_num_dofs), gymapi.DofState.dtype)
		default_dof_state["pos"] = default_dof_pos

		# send to torch
		self.default_dof_pos_tensor = to_torch(
			default_dof_pos, device=self.device).unsqueeze(dim=0).repeat((self.num_envs, 1, 1))
		...
		pose0 = gymapi.Transform()
		pose0.p = gymapi.Vec3(-0.6, -0.5, 0.4)
		pose0.r = gymapi.Quat(0.0, 0.0, np.sqrt(2)/2, np.sqrt(2)/2)
		pose1 = gymapi.Transform()
		pose1.p = gymapi.Vec3(0.6, 0.5, 0.4)
		pose1.r = gymapi.Quat(0.0, 0.0, np.sqrt(2)/2, -np.sqrt(2)/2)
		franka_pose = [pose0, pose1] 

		self.table_position = [[-0.5 * table_dims.x-0.1, 0.0, 0.5 * table_dims.z], [0.5 * table_dims.x+0.1, 0.0, 0.5 * table_dims.z]]
		pose0 = gymapi.Transform()
		pose0.p = gymapi.Vec3(*self.table_position[0])
		pose1 = gymapi.Transform()
		pose1.p = gymapi.Vec3(*self.table_position[1])
		table_pose = [pose0, pose1] 
		self.table_position = torch.tensor(self.table_position, device=self.device)
		...
		for i in range(self.num_envs):
			...
			for j in range(2):
				table_handle = self.gym.create_actor(
					env, table_asset, table_pose[j], f"table_{j}", i, 0)
				color = gymapi.Vec3(116 / 255, 142 / 256, 138 / 256)
				self.gym.set_rigid_body_color(
					env, table_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
				if i == 0:
					self.table_handle.append(table_handle)

			# add box
			box_pose.p.x = table_pose[0].p.x + np.random.uniform(-0.1, 0.3)
			box_pose.p.y = table_pose[0].p.y + np.random.uniform(-0.3, 0.3)
			...
			box_rs_props[0].friction = np.random.uniform(
				*self.cfg.domain_randomization.friction_range)
			...
			# add goal
			goal_pose.p.x = table_pose[1].p.x + np.random.uniform(-0.1, 0.3)
			goal_pose.p.y = table_pose[1].p.y + np.random.uniform(-0.3, 0.3)
			goal_pose.p.z = table_dims.z + 0.5 * box_size + 0.001
			goal_pose.r = gymapi.Quat.from_axis_angle(
				gymapi.Vec3(0, 0, 1), np.random.uniform(-math.pi, math.pi))
			goal_handle = self.gym.create_actor(env, goal_asset, goal_pose, "goal", i+self.num_envs, 0)
			if i == 0:
				self.goal_handle = goal_handle
				self.goal_position = [goal_pose.p.x, goal_pose.p.y, goal_pose.p.z]
			self.gym.set_rigid_body_color(
				env, goal_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)

			# get global index of box in rigid body state tensor
			goal_idx = self.gym.get_actor_rigid_body_index(
				env, goal_handle, 0, gymapi.DOMAIN_SIM)
			...
			for j in range(2):
				...
				if i == 0:
					self.franka_handle.append(franka_handle)
				...
				self.gym.set_actor_dof_position_targets(
					env, franka_handle, default_dof_pos)
				...
				init_rot_list.append([hand_pose.r.x, hand_pose.r.y,
														hand_pose.r.z, hand_pose.r.w])
				...
				lfinger_idx = self.gym.find_actor_rigid_body_index(
					env, franka_handle, "panda_leftfinger", gymapi.DOMAIN_SIM)
				self.lfinger_idxs.append(lfinger_idx)
				rfinger_idx = self.gym.find_actor_rigid_body_index(
			...
			if self.cfg.obs.type == "pixel":
				...
				if self.cfg.cam.view == "ego":
					...
					rand_loc_p = (np.array(self.cfg.cam.loc_p) + np.random.uniform(*
												self.cfg.cam.loc_shift_range, size=3)).tolist()
					...
					rigid_body_table_ind = self.gym.get_actor_rigid_body_handle(
						env, table_handle, 0)
					local_t = gymapi.Transform()
					...
					local_t.r = gymapi.Quat.from_euler_zyx(
						np.radians(0.0), np.radians(60.0), np.radians(180.0))
					self.gym.attach_camera_to_body(
	...
	def _parse_cfg(self, cfg: BaseConfig):
		...
		if cfg.control.controller == "cartesian_impedance":
			self.kp = torch.diag(torch.cat([torch.ones(
				3) * cfg.control.kp_translation, torch.ones(3) * cfg.control.kp_rotation]).float()).unsqueeze(dim=0)
			self.kd = torch.diag(torch.cat([torch.ones(
				3) * cfg.control.kd_translation, torch.ones(3) * cfg.control.kd_rotation]).float()).unsqueeze(dim=0)
	...
	def _init_buffer(self):
		...
		self.actor_ids_int32 = torch.arange(
			0, self.root_state.shape[0], dtype=torch.int32, device=self.device, requires_grad=False).reshape((self.num_envs, -1))
		# get jacobian tensor
		...
		# only need elements corresponding to the franka arm
		self.mm = mm[:, :7, :7]
		...
		self.dof_pos = self.dof_states[:, 0].view(self.num_envs, 2, 9, 1)
		self.dof_vel = self.dof_states[:, 1].view(self.num_envs, 2, 9, 1)
		...
		self.local_grasp_pos = torch.zeros(
			self.num_envs, 2, 3, dtype=torch.float, device=self.device)
		self.local_grasp_pos[..., 2] = 0.1034
		self.local_grasp_rot = torch.zeros(
			self.num_envs, 2, 4, dtype=torch.float, device=self.device)
		...
		if self.cfg.obs.type == "pixel":
			...
			for _ in range(self.cfg.obs.history_length):
				self.image_history.append(torch.zeros(
					(self.num_envs, 3 * self.cfg.obs.im_size * self.cfg.obs.im_size), dtype=torch.float, device=self.device))
			self.image_transform = torchvision.transforms.ColorJitter(
		...
		for _ in range(self.cfg.obs.state_history_length):
			self.state_history.append(torch.zeros(
				(self.num_envs, num_state), dtype=torch.float, device=self.device))

		# prepare obs
		self.target_eef_pos_obs = torch.zeros(
			(self.num_envs, 3), dtype=torch.float, device=self.device, requires_grad=False)

		self.noise_vec = torch.zeros(
			(1, num_state), dtype=torch.float, device=self.device, requires_grad=False)
		...
		self.motor_pos_target = torch.zeros(
			self.num_envs, 2, self.franka_num_dofs, dtype=torch.float, device=self.device, requires_grad=False)
		...
		# self.target_eef_orn = torch.tensor(
		#   [[0.924, -0.383, 0., 0.],[0.383, 0.924, 0., 0.]], dtype=torch.float, device=self.device, requires_grad=False).repeat((self.num_envs, 1))
		self.target_eef_orn = torch.tensor(
			[[0., 1., 0., 0.],[1., 0., 0., 0.]], dtype=torch.float, device=self.device, requires_grad=False).repeat((self.num_envs, 1))
		self.target_eef_pos = torch.zeros(
			(self.num_envs, 2, 3), dtype=torch.float, device=self.device, requires_grad=False)
		self.target_finger = torch.ones(
			(self.num_envs, 2, 1), dtype=torch.float, device=self.device, requires_grad=False)
	...
	def _init_default_dof(self):
		desired_x = self.table_position[:, 0] + torch.rand(
			(self.num_envs, 2), dtype=torch.float, device=self.device, requires_grad=False) * 0.6 - 0.3
		desired_y = self.table_position[:, 1] + torch.rand(
			(self.num_envs, 2), dtype=torch.float, device=self.device, requires_grad=False) * 0.4 - 0.2
		desired_z = 0.6 + torch.rand((self.num_envs, 2), dtype=torch.float,
																 device=self.device, requires_grad=False) * 0.2
		desired_pos = torch.stack([desired_x, desired_y, desired_z], dim=-1)
		for _ in range(10):
			pos_error = (desired_pos.view(-1, 3) - self.rb_states[self.hand_idxs, :3])
			rot_error = orientation_error(
	...
	def _reset_root_states(self, env_ids):
		...
		choosed_table_pos = self.table_position[(torch.rand(1)[0] < 0.5).long()]
		self.root_state[actor_ids_long, 0] = choosed_table_pos[0] + torch.rand(
			size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.4 - 0.2
		self.root_state[actor_ids_long, 1] = choosed_table_pos[1] + torch.rand(
			size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.4 - 0.2
		self.root_state[actor_ids_long, 2] = self.box_position[2]
		self.root_state[actor_ids_long, 3:5] = 0
		self.root_state[actor_ids_long, 5] = np.sqrt(2)/2
		self.root_state[actor_ids_long, 6] = np.sqrt(2)/2 
		self.root_state[actor_ids_long, 7:13] = 0
		# Randomize goal position
		goal_ids_int32 = self.actor_ids_int32[env_ids, self.goal_handle].flatten()
		goal_ids_long = goal_ids_int32.long()
		choosed_table_pos = self.table_position[(torch.rand(1)[0] < 0.5).long()]
		self.root_state[goal_ids_long, 0] = choosed_table_pos[0] + torch.rand(
			size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.4 - 0.2
		self.root_state[goal_ids_long, 1] = choosed_table_pos[1] + torch.rand(
			size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.4 - 0.2
		self.root_state[goal_ids_long, 2] = self.box_position[2]
		self.root_state[goal_ids_long, 3:5] = 0
		self.root_state[goal_ids_long, 5] = np.sqrt(2)/2
		self.root_state[goal_ids_long, 6] = np.sqrt(2)/2 
		self.root_state[goal_ids_long, 7:13] = 0
		actor_ids_int32 = torch.cat([actor_ids_int32, goal_ids_int32])
		if np.random.uniform() < self.goal_in_air:
			self.root_state[goal_ids_long, 2] += torch.rand(
				size=env_ids.shape, dtype=torch.float, device=self.device) * 0.4
		self.gym.set_actor_root_state_tensor_indexed(
			self.sim, gymtorch.unwrap_tensor(self.root_state), gymtorch.unwrap_tensor(
				actor_ids_int32), len(actor_ids_int32)
		)

	# def _resample_goals(self, env_ids):
	#   choosed_table_pos = self.table_position[(torch.rand(1)[0] < 0.5).long()]
	#   self.box_goals[env_ids, 0] = choosed_table_pos[0] + torch.rand(
	#     size=env_ids.shape, dtype=torch.float, device=self.device) * 0.4 - 0.1
	#   self.box_goals[env_ids, 1] = choosed_table_pos[1] + torch.rand(
	#     size=env_ids.shape, dtype=torch.float, device=self.device) * 0.6 - 0.3
	#   self.box_goals[env_ids, 2] = self.table_dims[2] + self.box_size / 2
	#   if np.random.uniform() < self.goal_in_air:
	#     self.box_goals[env_ids, 2] += torch.rand(
	#       size=env_ids.shape, dtype=torch.float, device=self.device) * 0.4

	def _set_dof_position(self, target_dof):
		actor_ids_int32 = self.actor_ids_int32[:,
																					 self.franka_handle].flatten().contiguous()
	...
	def step(self, actions):
		actions = torch.clone(actions).view(actions.shape[0], 2, actions.shape[1]//2)
		if actions.shape[2] == 5:
			action_type = actions[..., 0:1]
			pos_action = torch.where(action_type == 0, torch.clamp(
				actions[..., 1:4], -1.0, 1.0), torch.zeros_like(actions[..., 1:4]))
			gripper_action = torch.where(action_type == 1, torch.clamp(
				actions[..., 4:5], -1.0, 1.0), self.target_finger)
		elif actions.shape[2] == 4:
			pos_action = torch.clamp(actions[..., :3], -1.0, 1.0)
			gripper_action = torch.clamp(actions[..., 3:], -1.0, 1.0)
		eef_target = self.rb_states[self.hand_idxs, :3].view(self.num_envs, 2, 3) + 0.05 * pos_action
		filtered_pos_target = self.rb_states[self.hand_idxs, :3].view(self.num_envs, 2, 3)

		_, self.target_eef_pos_obs = tf_combine(
			self.rb_states[self.hand_idxs, 3:7].view(self.num_envs, 2, 4), eef_target,
		...
		for _ in range(self.cfg.control.decimal):
			filtered_pos_target = self.cfg.control.filter_param * eef_target + \
				(1 - self.cfg.control.filter_param) * filtered_pos_target
			pos_error = filtered_pos_target.view(-1,3) - self.rb_states[self.hand_idxs, :3]
			orn_error = orientation_error(
				self.target_eef_orn, self.rb_states[self.hand_idxs, 3:7])
			dpose = torch.cat([pos_error, orn_error], dim=-1).unsqueeze(dim=-1)
			if self.cfg.control.controller == "ik":
				self.motor_pos_target[..., :7] = self.dof_pos[..., :7,
																										0] + control_ik(dpose, self.j_eef, self.damping).view(self.num_envs, 2, 7)
				...
				self.effort_action[:, :7] = tensor_clamp(
					self.last_torques +
					torch.clamp(diff, -1000 * self.sim_params.dt,
											1000 * self.sim_params.dt),
				...
				self.effort_action[:, :7] = control_cartesian_pd(
					self.dof_pos[:, :7], self.dof_vel[:,
																						:7], tcp_pos, tcp_rot, self.j_eef,
					tcp_desired_pos, tcp_desired_rot,
					ee_vel_desired, ee_rvel_desired, self.kp, self.kd
				)
			# Clip
			target_finger_pos = 0.02 + 0.02 * gripper_action.repeat((1, 1, 2))
			self.motor_pos_target[..., 7:9] = self.dof_pos[..., 7:9, 0] + torch.clamp(
				target_finger_pos - self.dof_pos[..., 7:9, 0], min=-0.005, max=0.005)
	...
	def check_termination(self):
		...
		if self.cfg.safety.brake_on_contact:
			self.is_brake = torch.logical_or(self.net_cf[self.lfinger_idxs, 2] > self.cfg.safety.contact_force_th,
																			 self.net_cf[self.rfinger_idxs, 2] > self.cfg.safety.contact_force_th).view(self.num_envs, 2).any(dim=-1)
	...
	def compute_reward(self):
		...
		if self.cfg.reward.type == "sparse":
		...
		elif self.cfg.reward.type == 'sparse-':
			rew	= -torch.mean((distance > self.box_size).float(), dim=-1)
		elif self.cfg.reward.type == "dense":
			hand_rot = self.rb_states[self.hand_idxs, 3:7].view(self.num_envs, 2, 4)
			hand_pos = self.rb_states[self.hand_idxs, :3].view(self.num_envs, 2, 3)
			tcp_rot, tcp_pos = tf_combine(
				hand_rot, hand_pos,
				self.local_grasp_rot, self.local_grasp_pos
			)
			...
			bonus = torch.logical_and(
				distance < self.box_size, self.episode_length_buf > 1)
			finger_contact = torch.logical_or(self.net_cf[self.lfinger_idxs, 2] > self.cfg.safety.contact_force_th,
																				self.net_cf[self.rfinger_idxs, 2] > self.cfg.safety.contact_force_th).view(self.num_envs, 2).any(dim=-1)
			rew = torch.clamp(self.last_distance - total_distance, min=0) + \
				bonus.float() + self.cfg.reward.contact_coef * finger_contact.float()
	...
	def light_randomization(self):
		if self.common_step_counter % 1 == 0:
			...
			l_color = gymapi.Vec3(np.random.uniform(
				1, 1), np.random.uniform(1, 1), np.random.uniform(1, 1))
			...
			l_direction = gymapi.Vec3(np.random.uniform(
				0, 1), np.random.uniform(0, 1), np.random.uniform(0, 1))
	...
	def compute_observations(self):
		...
		if self.cfg.obs.type == "pixel":
			...
			for i in range(self.num_envs):
				crop_l = (self.cfg.cam.w -
									self.cfg.obs.im_size) // 2 if self.cfg.cam.crop == "center" else 0
				crop_r = crop_l + self.cfg.obs.im_size
				# TODO: apply random shift
				_rgb_obs = self.cam_tensors[i][:, crop_l:crop_r, :3].permute(
					2, 0, 1).float() / 255.
		...
		for i in range(len(self.state_history)):
			self.obs_buf[:, start_idx + i * state_dim: start_idx + (i + 1) * state_dim] = self.state_history[i][:, state_start_idx:] + torch.randn_like(
				self.noise_vec[:, state_start_idx:]) * self.noise_vec[:, state_start_idx]
