...
class PandaPickEnv(BaseTask):
  ...
  def _create_envs(self):
    ...
    self.default_dof_pos_tensor = to_torch(
      default_dof_pos, device=self.device).unsqueeze(dim=0).repeat((self.num_envs, 1))
    ...
    for i in range(self.num_envs):
      ...
      franka_handle = self.gym.create_actor(
        env, franka_asset, franka_pose, "franka", i, 2)
      if i == 0:
  ...
  def _init_buffer(self):
    ...
    self.motor_pos_target = torch.zeros(
      self.num_envs, self.franka_num_dofs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    self.target_eef_orn = torch.tensor(
      [[1.0, 0., 0., 0.]], dtype=torch.float, device=self.device, requires_grad=False).repeat((self.num_envs, 1))
    ...
    self.box_goals = torch.zeros_like(
      self.rb_states[self.box_idxs, :3], requires_grad=False)
  ...
  def _init_default_dof(self):
    desired_x = self.table_position[0] + torch.rand(
      (self.num_envs,), dtype=torch.float, device=self.device, requires_grad=False) * 0.4 - 0.1
    desired_y = self.table_position[1] + torch.rand(
      (self.num_envs,), dtype=torch.float, device=self.device, requires_grad=False) * 0.6 - 0.3
    desired_z = 0.6 + torch.rand((self.num_envs,), dtype=torch.float,
  ...
  def reset_idx(self, env_ids):
    ...
    if self.cfg.reward.type == "dense":
      ...
      tcp2obj = torch.norm(
        tcp_pos - self.rb_states[self.box_idxs, :3][env_ids], dim=-1)
  ...
  def _reset_root_states(self, env_ids):
    ...
    self.root_state[actor_ids_long, 0] = self.table_position[0] + torch.rand(
      size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.4 - 0.1
    self.root_state[actor_ids_long, 1] = self.table_position[1] + torch.rand(
      size=actor_ids_int32.shape, dtype=torch.float, device=self.device) * 0.6 - 0.3
  ...
  def _resample_goals(self, env_ids):
    self.box_goals[env_ids, 0] = self.table_position[0] + torch.rand(
      size=env_ids.shape, dtype=torch.float, device=self.device) * 0.4 - 0.1
    self.box_goals[env_ids, 1] = self.table_position[1] + torch.rand(
    ...
    for i in range(self.cfg.control.decimal):
      ...
      if self.cfg.control.controller == "ik":
        self.motor_pos_target[:, :7] = self.dof_pos[:, :7,
                                                    0] + control_ik(dpose, self.j_eef, self.damping)
  ...
  def compute_observations(self):
    ...
    self.state_history[-1][:, 3:] = torch.cat([
      tcp_pos, tcp_rot, self.dof_pos[:, 7:9,
                                     0], self.target_eef_pos_obs, self.box_goals
    ], dim=-1)
